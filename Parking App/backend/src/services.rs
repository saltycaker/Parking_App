...
impl ParkingService {
    ...
    pub async fn search_parking(
        ...
        // Sort by distance
        results.sort_by(|a, b| a.distance_m.partial_cmp(&b.distance_m).unwrap_or(std::cmp::Ordering::Equal));

        let total = results.len() as i32;

        // Clone query before moving request fields
        let query_str = request.query.clone().unwrap_or_else(|| "nearby parking".to_string());
        let req_latitude = request.latitude;
        let req_longitude = request.longitude;
    ...
    async fn calculate_availability(&self, db: &Database, location: &ParkingLocation) -> AppResult<AvailabilityEstimate> {
        ...
        if weekday == chrono::Weekday::Sat || weekday == chrono::Weekday::Sun {
            score += 15; // Weekend - higher availability
        } else {
            score -= 5; // Weekday - lower availability
        }
    ...
    async fn save_search_history(&self, db: &Database, user_id: Uuid, request: &SearchRequest, results_count: i32) -> AppResult<()> {
        let radius = request.radius_m.unwrap_or(self.config.default_search_radius_m);
        sqlx::query(
            "INSERT INTO search_history (id, user_id, query, latitude, longitude, radius_m, results_count, created_at)
    ...
    pub async fn get_recommendations(
        ...
        let _history = sqlx::query_as::<_, SearchHistoryEntry>(
        ...
        // Search nearby for recommendations
        ...
        let search_results = self.search_parking(db, cache, search_request, Some(user_id)).await?;
